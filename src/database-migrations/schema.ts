import { pgTable, pgSchema, index, check, uuid, text, timestamp, uniqueIndex, foreignKey, jsonb, boolean, varchar, bigserial, smallint, json, inet, serial, date, integer, unique, real, bigint, numeric } from "drizzle-orm/pg-core"
...
export const conversations = pgTable("conversations", {
	...
	rehomerLastOpenedChatAt: timestamp("rehomer_last_opened_chat_at", { withTimezone: true, mode: 'string' }),
	rehomerLastClosedChatAt: timestamp("rehomer_last_closed_chat_at", { withTimezone: true, mode: 'string' }),
	adopterLastOpenedChatAt: timestamp("adopter_last_opened_chat_at", { withTimezone: true, mode: 'string' }),
	adopterLastClosedChatAt: timestamp("adopter_last_closed_chat_at", { withTimezone: true, mode: 'string' }),
	createdAt: timestamp("created_at", { withTimezone: true, mode: 'string' }).notNull(),
	lastMessageSentAt: timestamp("last_message_sent_at", { withTimezone: true, mode: 'string' }),
...
export const swipes = pgTable("swipes", {
	...
	swipedAt: timestamp("swiped_at", { withTimezone: true, mode: 'string' }).notNull(),
	rehomerId: uuid("rehomer_id").notNull(),
...
export const userSearchPreferences = pgTable("user_search_preferences", {
	userSearchPreferenceId: serial("user_search_preference_id").primaryKey().notNull(),
...
export const animalsAdopted = pgTable("animals_adopted", {
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	adoptionId: bigint("adoption_id", { mode: "number" }).primaryKey().generatedByDefaultAsIdentity({ name: "animals_adopted_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 9223372036854775807, cache: 1 }),
	createdAt: timestamp("created_at", { withTimezone: true, mode: 'string' }).notNull(),
	animalId: integer("animal_id").notNull(),
	rehomerId: uuid("rehomer_id").notNull(),
}, (table) => [
...
export const animals = pgTable("animals", {
	...
	addressLatitude: numeric("address_latitude", { precision: 8, scale:  6 }).notNull(),
	addressLongitude: numeric("address_longitude", { precision: 9, scale:  6 }).notNull(),
	rehomerId: uuid("rehomer_id").notNull(),
}, (table) => [
	foreignKey({
			columns: [table.rehomerId],
